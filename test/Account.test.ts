...
describe('Wallet', () => {
  ...
  describe('transaction queue', function (this: {
    account: Account
  }) {
    beforeEach(() => {
      this.account = new Account({ privateKey: private_key_testnet_0 })
    ...
    test('can queue a transaction', async () => {
      ...
      const transactionPromise = this.account.signTransaction(transactionRequest)
    ...
    test('can queue many transaction', async () => {
      const transactionRequest1: TransactionRequest = { to: address_testnet_1, data: '0x1234' }
      const transactionRequest2: TransactionRequest = { to: address_testnet_1, data: '0x5678' }

      const transactionPromise1 = this.account.signTransaction(transactionRequest1)
      const transactionPromise2 = this.account.signTransaction(transactionRequest2)
